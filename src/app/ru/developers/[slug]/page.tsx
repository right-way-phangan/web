  ...
  return (
    <>
      {name ? (
        <BreadcrumbJsonLd
          crumbs={[
            { name: "Главная", url: `${base}/ru` },
            { name: "Проекты", url: `${base}/ru/projects` },
            { name, url: `${base}/ru/developers/${slug}` },
          ]}
        />
      ) : null}
      {profile ? (
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: jsonLdHtml({
              ...
              "@type": "Organization",
              name: profile.name,
              description: profile.seo?.description?.ru,
              url: `${base}/ru/developers/${slug}`,
            }),
          }}
        />
      ) : null}
      <DeveloperPage slug={slug} locale="ru" />
    </>
  );
}
