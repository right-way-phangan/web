...
// окупаемость печатала конкретный год | ОЖИДАЛОСЬ: если вложенное потеряно
// целиком, ни одна строка не обещает выхода в плюс | БЫЛО: `paybackFrom`
// возвращал ПЕРВЫЙ год, где накопленная прибыль стала неотрицательной, и не
// смотрел, что было дальше: у лизхолда кривая ненадолго выходит в плюс
// (пик ≈ +฿226K на 11-м году) и падает до −฿10.8M, поэтому рядом с «ROI −114.3%»,
// «CAGR —» и «IRR —» стоял KPI «Окупаемость 7.1 лет» и вердикт «прибыль с 7.1 года»
// | ИСПРАВЛЕНО 2026-09-05: год окупаемости выдаётся только если к концу срока
// вложенное реально вернулось (последняя точка серии неотрицательна)
...
describe("АТАКА 55 — «прибыль с 7.1 года» на сценарии, где потеряно всё вложенное", () => {
  ...
  it("и окупаемость больше не обещает года — вложенное не вернулось", () => {
    expect(wiped.paybackYears).toBeNull();
  });

  it("КОНТРОЛЬ: на прибыльном сценарии год окупаемости по-прежнему считается", () => {
    const ok = computeRoi({ ...DEFAULT_INPUTS, annualGrowthPct: 8, years: 15 });
    expect(ok.series[ok.series.length - 1].profit).toBeGreaterThan(0);
    expect(ok.paybackYears).not.toBeNull();
  });
