...
const KEY = "rw-estate-currency";

/** Валюты переключателя на странице участков (฿/$/₽). */
export const ESTATE_CURRENCIES: Currency[] = ["THB", "USD", "RUB"];
const SYMBOL: Record<Currency, string> = { THB: "฿", USD: "$", EUR: "€", RUB: "₽" };

/**
 * Провайдер валюты для страницы участков: цены лотов хранятся в THB, здесь —
 * только отображение. Живой курс тянем `fetchRates` (open.er-api, THB-база) с
 * откатом на `DEFAULT_RATES`. Выбор запоминаем в localStorage. Вне провайдера
 * хук безопасно отдаёт THB-форматирование (для серверных мест).
 */
export function EstateCurrencyProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      if (saved && ESTATE_CURRENCIES.includes(saved)) setCurrencyState(saved);
    } catch {
      /* приватный режим */
    }
...
export function useEstateCurrency(): Ctx {
  return (
...
export function CurrencyToggle({ className }: { className?: string }) {
  const { currency, setCurrency } = useEstateCurrency();
  return (
