 ...
 */
export function ProjectNav({ items }: { items: NavItem[] }) {
  ...
  return (
    <div className="sticky top-16 z-30 -mx-4 border-b border-forest-500/10 bg-cream-100/85 backdrop-blur-md md:top-20">
      <nav
        ...
        className="container-prose flex gap-1 overflow-x-auto px-4 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      >
        {items.map((item) => (
          <a
            key={item.id}
