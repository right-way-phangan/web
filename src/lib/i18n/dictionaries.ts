...
const process: Record<Locale, ProcessDict> = {
  ...
  ru: {
    ...
    steps: [
      ...
      { number: 6, title: "Договор купли-продажи", text: "Двуязычный договор (англ.–тайск.) от нашей партнёрской юрфирмы. Разбираем с вами каждый пункт и объясняем тайские нюансы простым языком. Ваш задаток держит юрфирма на отдельном клиентском счёте — не передаётся продавцу заранее — и выдаётся только при закрытии; для крупных сделок можем организовать лицензированный банковский эскроу-счёт." },
